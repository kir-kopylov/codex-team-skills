...
Запрещено оставлять человекочитаемое описание PR на английском вроде `What changed`, `Why`, `Validation`, `Notes`. Каркас описания задаёт раздел «Описание PR» в `CONTRIBUTING.md`: `Проблема`, `Что изменилось`, `Границы`, `Доказательства`.
