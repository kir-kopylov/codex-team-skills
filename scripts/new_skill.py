...
def main() -> None:
    ...
    parser.add_argument(
        "--author-github",
        default="@author",
        help="Подтверждённый GitHub-аккаунт автора для метаданных реестра",
    )
    parser.add_argument(
        "--source-asset",
...
Применяю черновой навык **«{args.display_name}»** (обратная связь — {args.owner}): <кратко назовите конкретную пользу для текущего запроса>; продолжаю без ожидания.
