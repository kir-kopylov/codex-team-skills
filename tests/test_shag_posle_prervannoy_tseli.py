...
SKILL = ROOT / "plugins" / "team-skills" / "skills" / "shag-posle-prervannoy-tseli"
...
def test_resume_skill_keeps_neighbor_ownership_boundaries() -> None:
    ...
    assert "narabotki-sessii-v-vetku" in content
    assert "kontrakt-tseli-do-starta" in content
    assert "sverka-git-pered-deystviem" in content
...
def test_resume_skill_keeps_mandatory_automatic_survey() -> None:
    ...
    assert "Что в этом использовании shag-posle-prervannoy-tseli было полезно?" in content
...
def test_resume_skill_has_openai_interface_metadata() -> None:
    ...
    assert "$shag-posle-prervannoy-tseli" in interface["default_prompt"]
