...
SKILL_DIR = ROOT / "plugins" / "team-skills" / "skills" / "dobavlenie-navyka-v-biblioteku"
...
def test_add_team_skill_registry_covers_observed_failure_classes():
    ...
    assert {
        ...
        "dobavlenie-navyka-v-biblioteku-self-probe-recursion",
    } <= ids
...
def test_add_team_skill_batches_changed_first_response_probes_privately():
    ...
    for freshness_fragment in (
        ...
        "`dobavlenie-navyka-v-biblioteku` отсутствует",
        "Третьего уровня нет",
        "внешняя строка `dobavlenie-navyka-v-biblioteku` получает обычный",
    ...
    assert "Внешний реальный пакет включает `dobavlenie-navyka-v-biblioteku`" in normalized_example
    assert "внутреннем diff есть два других существенно изменённых skill" in normalized_example
    assert "нет `dobavlenie-navyka-v-biblioteku`" in normalized_example
