...
def test_goal_contract_shaper_requires_complete_execution_chain() -> None:
    ...
    for required in (
        ...
        "Блок `Что будет достаточным ответом` не может впервые вводить нового исполнителя, канал, аккаунт, полномочие, внешнее действие или ручной шаг пользователя",
        "Он только показывает форму ответа на уже заданный вопрос и не вводит звенья, которых нет в принятом контракте или самом вопросе",
    ):
