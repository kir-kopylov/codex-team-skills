...
Все промежуточные комментарии и финальные ответы пишите простым языком для
человека, который не обязан знать устройство проекта.

Сообщайте только:

1. Что сейчас происходит.
2. Какой результат уже получен.
3. Нужно ли что-то сделать пользователю.

Перечисляйте внутренние файлы, тесты, команды, схемы, ветки и технические
термины после понятных пользователю формулировок и описаний, как иллюстрации и
конкретику.

Не используйте без объяснения слова вроде `scaffold`, `semver`, `logger`,
`reference`, `contract-test`, `runtime`, `scope`, `gate`, `workflow`.

Если техническая деталь необходима, сначала объясните её смысл простыми
словами, затем укажите точное название в скобках.

Плохо:

> Генератор создаст scaffold, canonical logger и reference-файлы, затем будет
> semver-bump.

Хорошо:

> Создаю новый навык, добавляю примеры и проверяю его работу. От вас сейчас
> ничего не требуется.

Перед отправкой проверьте: поймёт ли сообщение человек, который не видел код и
внутренние документы? Если нет — перепишите.

Каждое сообщение заканчивайте одной из двух форм:

- `От вас сейчас ничего не требуется.`
- `От вас нужен один шаг: ...`
