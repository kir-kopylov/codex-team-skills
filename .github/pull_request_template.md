...
- [ ] Старая очистка в `dry-run` ничего не меняет, повторный `apply` возвращает `NOT_FOUND`.
- [ ] Откат или повторная установка проверены.

## Проверки

- [ ] Заголовок и описание PR проходят metadata gate: русский пользовательский текст, разрешённая латиница, приватные маркеры и поле «Когда не использовать».
- [ ] Комментарии и ревью написаны на русском; технические имена, пути и команды оставлены как есть.
- [ ] `python -m pytest` проходит.
- [ ] Если менялись тесты: в PR объяснено, какой инвариант теста сохранён, уточнён или сознательно изменён, и почему новая проверка не даёт ложноположительный результат.
