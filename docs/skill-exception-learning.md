...
Прогон петли целиком: 2026-09-20 разобраны все 82 приватные карточки одной машины (76 карточек сбоев и 6 ответов опроса) из 27 каталогов `~/.codex/skill-runs/`. Каждая карточка получила вердикт: 49 карточек сбоя стали правилами (49 новых записей в `known-exceptions.yaml` 18 навыков), одно пожелание из опроса — правкой `SKILL.md`, 32 карточки отклонены с причиной (дубль существующего правила, разовый случай, не вина навыка, навык вне библиотеки). Автоматического harvest по-прежнему нет: прогон запускает человек, внесённые правки проверял независимый судья в отдельном контексте, а карточки и реестр вердиктов остались вне repo.
...
Оба канала приватны, оба разбирает `pravilo-iz-zhurnala-sboev`, и в repo из них попадает только очищенное знание через human approval.
...
Пожелания из `usage-feedback.jsonl` разбирает тот же `pravilo-iz-zhurnala-sboev`: повторяющееся или дешёвое в реализации пожелание становится правкой `SKILL.md` или example в patch proposal; `known-exceptions.yaml` при этом не трогается — он только для сбоев. Честно о зрелости: разбор фидбека запускается вручную (пользователь просит «разбери фидбек по skill»), автоматического harvest нет. В прогоне 2026-09-20 из шести ответов опроса одно пожелание стало правкой `SKILL.md`, остальные отклонены как дубли действующих правил или разовые.
...
`pravilo-iz-zhurnala-sboev` читает приватные карточки или sanitized packets и предлагает patch proposal. Он не применяет patch сам.
