...
Пожелания из `usage-feedback.jsonl` разбирает тот же `pravilo-iz-zhurnala-sboev`: повторяющееся или дешёвое в реализации пожелание становится правкой `SKILL.md` или example в patch proposal; `known-exceptions.yaml` при этом не трогается — он только для сбоев. Честно о зрелости: разбор фидбека сегодня запускается вручную (пользователь просит «разбери фидбек по skill»), автоматического harvest нет.
