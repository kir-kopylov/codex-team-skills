---
name: kontrakt-tseli-do-starta
description: "«Проверь цель для /goal», «сформулируй /goal-контракт», «сделай сырую цель проверяемой», «добавь completion gate»: ценность, роли, потери. Задачу не выполняет."
---
...
7. **Память**: назовите хранилище, сохраняемый результат единицы, статус и порядок возобновления. Выберите достаточный режим по последствиям действий, а не только по длительности цели.

   Для одного исполнителя, который читает источники без внешних последствий и сохраняет воспроизводимые локальные результаты, достаточно учёта единиц в одном месте, например таблицы в отчёте. Каждое чтение должно допускать безопасный повтор; платные запросы, списание ограниченной квоты и расход одноразового доступа требуют полного режима. Укажите идентификатор единицы, указатель на исходник и результат, статус и следующий шаг. Сначала сохраните и проверьте результат, затем отмечайте единицу готовой. После обрыва сверьте записи с сохранённым, продолжайте незавершённое и не создавайте дубли; отсутствие или расхождение результата требует повторной сверки, а не отметки «готово». Счётчики выводите из этого учёта. Отдельные журнал, контрольная точка и запись намерения перед каждым чтением здесь не обязательны. Охват, источники выводов и финальная приёмка остаются обязательными.

   Полный режим нужен для чтений с внешними последствиями или небезопасным повтором, отправок, покупок, публикаций и иных внешних изменений, невоспроизводимых результатов, нескольких писателей либо обязательной истории действий. При неясных последствиях сначала установите границу; неизвестность не разрешает упрощение. Уже действующий журнал или обязательные правила runtime не заменяйте отчётом.

   В полном режиме контракт называет: физическое хранилище (путь или место, переживающее обрыв сессии), состав артефактов (журнал, контрольная точка состояния, отчет), порядок записи (write-ahead: сначала журнал, потом действие), протокол возобновления после сна/выключения/обрыва, пересчет дедлайнов по календарному времени от меток журнала и защиту от повторных внешних действий при возобновлении. Append-only журнал остаётся единственным источником правды, а контрольная точка хранит последнее применённое событие или смещение, проверочный отпечаток, восстановленное состояние и следующий шаг. При обычном старте или возобновлении исполнитель загружает контрольную точку и читает только хвост журнала после неё. Полный повторный проход допустим только при отсутствии или недействительности контрольной точки, явном аудите либо восстановлении после повреждения; перед обычной записью запрещено перечитывать весь журнал. Сводные поля (счётчики, статусы) выводятся из журнала, а не пишутся независимо в разных артефактах; данные, что уже на руках, не хранятся как «?». Перед повтором внешнего действия исполнитель проверяет его актуальное состояние в авторитетном источнике, а не ретраит вслепую — слепой повтор даёт дубли записей и мис-клики. Для цели длиннее одной сессии вопросы о месте хранения и допустимых сроках задаются пользователю, а устройство обратимой контрольной точки внутри принятой области можно выбрать и записать как рабочую техническую деталь.
...
- учёт прогресса без статуса и следующего шага;
- учёт прогресса без физического носителя и проверяемого протокола возобновления после обрыва;
...
- сводные счётчики, писанные независимо в артефактах, а не выведенные из выбранного единого учёта; слепой повтор внешнего действия без перечитывания состояния;
...
- в полном режиме обычное возобновление требует полного чтения растущего журнала вместо контрольной точки и хвоста;
- упрощённый учёт выбран при чтениях с внешними последствиями или небезопасным повтором, внешних изменениях, невоспроизводимых результатах, нескольких писателях или обязательной истории действий;
...
Для цели «собрать источники → разобрать случаи → обобщить опыт → предложить
правки» используйте [шаблон аналитической цели](references/analytical-goal-template.md).
Он помогает оформить этот маршрут в пределах уже выбранных полномочий;
чтение шаблона не запускает сбор и не разрешает применять предлагаемые правки.
