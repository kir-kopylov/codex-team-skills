---
name: podsadka-nezvanyh-v-snimok
description: "«Сделай фотобомбинг», «добавь людей или животных», «усиль абсурд», «мемнее», «ещё уровень», «детский/пенсионный вайб». Слои шутки на фото без правки людей и фона."
---
...
Применяю **«Режиссёр фотобомбинга»**: <кратко назовите конкретную дополнительную процедуру или проверяемый результат для текущего запроса>; продолжаю без ожидания.
...
Пользователь не должен писать `$podsadka-nezvanyh-v-snimok`.
...
```text
Опрос по skill:
1. Что в этом использовании podsadka-nezvanyh-v-snimok было полезно?
2. Что стоит доработать в skill или его формате?
Можно ответить коротко или написать "пропустить".
```

Если пользователь ответил, сохраните санированную карточку в `~/.codex/skill-runs/podsadka-nezvanyh-v-snimok/usage-feedback.jsonl` — лучше через bundled script:
...
- "Apply $podsadka-nezvanyh-v-snimok: 3 layers, child-safe, joy 8, grotesque 1" -> явный вызов тоже работает.
