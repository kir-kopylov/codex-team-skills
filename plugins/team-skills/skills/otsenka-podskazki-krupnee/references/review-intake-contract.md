...
`otsenka-podskazki-krupnee` - offline/manual evaluator. Он не является watcher, telemetry daemon, CI service или автоматическим наблюдателем runtime.
...
```text
У тебя новые feedback rows по podskazka-krupnee-posle-povtorov. Разобрать?
```
