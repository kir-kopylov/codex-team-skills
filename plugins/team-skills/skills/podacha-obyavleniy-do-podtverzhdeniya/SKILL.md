---
name: podacha-obyavleniy-do-podtverzhdeniya
description: "«Размести объявления на OLX/Kaspi», «лоты даром самовывозом», «забрать сегодня», «не перехватывай клавиатуру», «через Chrome API», «Chronicle». Фон, gate до публикации."
---
...
Применяю **«Фоновая подготовка объявлений»**: <кратко назовите конкретную дополнительную процедуру или проверяемый результат для текущего запроса>; продолжаю без ожидания.

Не включайте в строку `author_github`, внутреннее имя папки или пересказ всего запроса. Не спрашивайте, применять ли навык.

Не завершайте первый ответ уведомлением или обещанием будущей работы. Сразу начните доступную read-only инвентаризацию лотов, фото и открытой сессии площадки. Если источник лотов нельзя определить из текущего контекста или инструментов, в том же ответе задайте один ближайший вопрос о папке, документе или другом источнике лотов.
...
```text
Опрос по skill:
1. Что в этом использовании podacha-obyavleniy-do-podtverzhdeniya было полезно?
2. Что стоит доработать в skill или его формате?
Можно ответить коротко или написать "пропустить".
```

Если пользователь ответил, сохраните санированную карточку в `~/.codex/skill-runs/podacha-obyavleniy-do-podtverzhdeniya/usage-feedback.jsonl` — лучше через bundled script:
