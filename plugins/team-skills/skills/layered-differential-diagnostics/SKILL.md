---
name: layered-differential-diagnostics
description: "Используйте этот экспериментальный skill при первом неоднозначном техническом сбое, когда один симптом допускает несколько причин: «разбери сбой по слоям», «какой один тест различит причины», «не гадай, а проверь», «в одном месте работает, в другом нет». Skill отделяет пользовательский результат от косвенных признаков, строит две конкурирующие гипотезы и выбирает один безопасный причинно-различающий опыт. Самостоятельно выполняет только безопасные проверки; изменения среды требуют отдельного разрешения."
---

# Послойная Различающая Диагностика

## Согласие На Запуск

Явный вызов — slash-команда, имя skill или первая фраза из каталога — выполняйте сразу, без вопроса. При автосрабатывании на смысловое сходство сначала спросите одной строкой: «Задача похожа на экспериментальный team skill `layered-differential-diagnostics` от `@kir-kopylov` — отделяет факты от гипотез и выбирает один безопасный различающий опыт. Применить или решить без него?» — и ждите ответа. При отказе выйдите из skill молча: решите задачу с нуля и больше не упоминайте skill.
...
2. Зафиксируйте outcome, прямое доказательство успеха и forbidden false
   ...
   результат.
3. Соберите свежий `state_fingerprint`: целевой объект, время, релевантная
   конфигурация, среда, последний шаг, наблюдение и происхождение evidence.
...
- существующие тесты и проверки, которые не переписывают отслеживаемые файлы;
...
```text
Опрос по skill:
1. Что в этом использовании layered-differential-diagnostics было полезно?
...
`~/.codex/skill-runs/layered-differential-diagnostics/usage-feedback.jsonl` —
...
`~/.codex/skill-runs/layered-differential-diagnostics/exception-log.jsonl`.
