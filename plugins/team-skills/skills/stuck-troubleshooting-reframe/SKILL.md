...
При автосрабатывании сначала проверьте, что в запросе есть зацикленная диагностика: названы недостигнутый outcome и повторённые failed/partial попытки или ложные зелёные сигналы. Одна просьба найти чужой опыт без repair-loop не является смысловым автотриггером: не показывайте карточку и передайте запрос в отдельный workflow поиска внешней практики.
...
- Пользователь просит только найти, как другие решали похожую проблему, а повторённого local no-outcome нет: не запускайте reframe; используйте отдельный workflow поиска внешней практики.
...
5. `Reframe Eligibility Gate`: если `same_state_count < 2` и нет повторного no-outcome при ложных зелёных сигналах, вернитесь к обычной диагностике. Сама по себе просьба найти похожие кейсы не открывает reframe.
6. `External Case Gate`: примите до 5 внешних кандидатов из отдельного workflow. Каждый кейс должен дать локально проверяемый observable; его внешний статус не доказывает локальную причину. Если кандидата нет, а новый слой уже следует из локальных фактов, отметьте `external check: not performed`. Если без внешнего кандидата новый слой не выбрать, остановитесь до action и передайте один запрос в отдельный workflow; не начинайте открытый веб-поиск внутри skill.
