...
- конкретный объект: сломанная функция, приложение, соединение, процесс или другой результат, которого не удаётся достичь;
- запрошенное количество: число повторившихся сбоев, уже проверенных веток, похожих случаев или следующих экспериментов, если оно указано;
- проверяемые сведения: критерий успеха, текущее наблюдаемое состояние, точные ошибки, проверенные гипотезы, ложноположительные сигналы, ограничения и действия, которые нельзя повторять.
...
Для вашей задачи — <кратко повторите действие, недостигнутый результат, количество повторов или веток и проверяемые сведения> — может пригодиться командный навык **«Выход из зацикленной диагностики»**.
...
> **С навыком**
>
> <В двух–трёх полных предложениях снова назовите действие пользователя, недостигнутый результат, запрошенное количество повторившихся сбоев, уже проверенных веток, похожих случаев или следующих экспериментов и все проверяемые сведения. Объясните, что отделите критерий успеха от промежуточных зелёных сигналов, зафиксируете состояние и закрытые ветки, сопоставите похожие случаи и предложите один эксперимент на новом уровне с ожидаемым наблюдением, признаком опровержения, откатом и условием остановки.>

> **Без навыка**
>
> <В двух–трёх полных предложениях снова назовите то же действие пользователя, тот же недостигнутый результат, то же количество повторившихся сбоев, уже проверенных веток, похожих случаев или следующих экспериментов и те же проверяемые сведения. Объясните, что обычный ответ может предложить ещё одну диагностическую проверку, но не гарантирует закрытие повторяющейся ветки, различение конечного результата и промежуточного сигнала и переход к новому уровню с явным условием опровержения.>

**<Одним полным предложением объясните, что переосмысление требует наблюдаемого текущего состояния и истории проверок, применяется после повторения одного уровня или по прямому запросу на похожие случаи и не гарантирует, что новый эксперимент сразу устранит сбой.>**
