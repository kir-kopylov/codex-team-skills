...
2. **Сформулируйте остаточный контракт одной фразой.** Что пользователь сможет делать после удаления и каким единственным поддерживаемым путём. Пример: «Новые версии ставятся повторным ручным запуском подписанного installer; фоновых задач нет».
...
8. **Докажите остаточный контракт.** Запустите реальный smoke поддерживаемого пути в заявленном runtime. Статический поиск и parse check не доказывают, что installer, CLI или ручной workflow действительно работает.
