...
Явный вызов — slash-команда, имя skill или первая фраза из каталога — выполняйте сразу, без вопроса. При автосрабатывании на смысловое сходство сначала спросите одной строкой: «Задача похожа на экспериментальный team skill `windows-app-connectivity-split-brain` от `@kir-kopylov` — разводит сетевые слои, когда конкретное Windows-приложение offline при спорном VPN/proxy/DNS состоянии. Применить или решить без него?» — и ждите ответа. При отказе выйдите из skill молча: решите задачу с нуля и больше не упоминайте skill.
