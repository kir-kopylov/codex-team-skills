...
В complexity ledger updater получает `delete`: отдельная пользовательская боль для фонового автообновления не доказана, а его дополнительный ownership cost включает scheduler, фоновую orchestration, state/repair, rollback, Windows-совместимость и поддержку legacy-задач. Подписанный release и повторный ручной installer получают отдельный `keep`: они закрывают доставку без updater runtime.
...
Нельзя продолжать строить Windows E2E «раз уж начали». Нельзя сохранять updater ради sunk cost. Нельзя расширять удаление на ручной installer или на несвязанные release-механизмы.
