...
description: "«Рассылка сработала?», «почему письма нет?», «не было ли дубля?», «засчитать этот запуск?», «подтверди плановую доставку»: цепочка запуск → отправка → провайдер → копия."
---
