...
При автосрабатывании на смысловое сходство сначала извлеките из текущего запроса действие пользователя, конкретную коллекцию файлов, заметок, закладок или шаблонов, запрошенное количество объектов или папок, если оно есть, и проверяемые сведения: текущее расположение, способы поиска, дубли, новые элементы, запреты на перемещение и желаемая структура.
...
Для вашей задачи — <кратко повторите действие пользователя, конкретную коллекцию, запрошенное количество объектов или папок и проверяемые сведения> — может пригодиться командный навык **«Разбор информационного беспорядка»**.
...
> **С навыком**
>
> <Полными предложениями снова назовите действие пользователя, коллекцию, количество объектов или папок и проверяемые сведения. Объясните, что сначала будет собрана инвентаризация, затем владелец ответит на короткие вопросы по живым примерам, получит две структуры и наглядную карту перемещений, а после выбора будет выполнена проба на одном объекте и только затем утверждённая перекладка.>

> **Без навыка**
>
> <Полными предложениями снова назовите то же действие пользователя, ту же коллекцию, количество объектов или папок и проверяемые сведения. Объясните, что обычный ответ может предложить названия папок и общие правила сортировки, но не гарантирует инвентаризацию фактического содержимого, карту будущих перемещений, проверку структуры на реальных объектах и остановку до утверждения владельца.>

**Навык не освобождает место на диске и ничего не удаляет; без доступа к коллекции он может подготовить только схему, а перемещения начинаются лишь после явного согласия на конкретную структуру.**
...
Каждый вопрос обязан менять результат; вопросы «для полноты» запрещены. Формат — раунды по 3–4 связанных вопроса с вариантами ответов: живые примеры нужно показывать пачкой, а раунды экономят владельцу время. Самые важные первыми:
...
2. **Живые примеры (обязательно):** вытащите 3 реальных объекта из разных частей корпуса и спросите про каждый: «вот это — куда должно лечь?» Ответы становятся контрольными проверками результата. Абстрактно спрашивать бесполезно — владелец сам не знает свою ось, пока не увидит конкретную вещь.
...
- Сомнение между двумя папками → решить по ответам владельца на живых примерах и оси поиска; если сигнала нет — вынести в следующий раунд вопросов или положить в более крупный домен, записав допущение в `ASSUMPTIONS.md`.
