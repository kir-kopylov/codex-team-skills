...
Явный вызов — slash-команда, имя skill или первая фраза из каталога — выполняйте сразу, без вопроса. При автосрабатывании на смысловое сходство сначала спросите одной строкой: «Задача похожа на экспериментальный team skill `razbor-bardaka` от `@kir-kopylov` — превращает информационную свалку в понятную структуру папок через карту и утверждение владельца. Применить или решить без него?» — и ждите ответа. При отказе выйдите из skill молча: решите задачу с нуля и больше не упоминайте skill.
...
- `inventory.csv` — каждый объект: стабильный id (id из базы, полный путь или URL), текущая папка (если есть), имя, тип, дата, выдержка. Полные тексты — в `items/<id>.txt`, только для текстовых объектов.
