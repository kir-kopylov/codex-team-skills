...
1. После окончательного staging успешно обновите remote refs, покажите точный
   `git diff --cached` и зафиксируйте OID staged tree из `git write-tree`, OID
   удалённых base и target либо подтверждённое отсутствие target.
2. Получите явное одобрение этого снимка. Перед `commit` и перед `push` снова
   обновите remote refs и сравните все три значения. Ошибка обновления или
   любое изменение запрещает `commit`, `push`, создание и обновление PR до
   нового показа и одобрения.
3. Tree commit должен совпасть с одобренным tree. После `push`, но до работы с
   PR, снова успешно обновите remote refs: OID base должен остаться одобренным,
   а OID target — указывать ровно на этот commit. Создайте или обновите PR
   только для этих веток и прочитайте его обратно: OID head PR должен совпасть
   с проверенным OID target, OID base PR — с одобренным OID base. Иначе снова
   покажите кандидата и получите новое одобрение.
