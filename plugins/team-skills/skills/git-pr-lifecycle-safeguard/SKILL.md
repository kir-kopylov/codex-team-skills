...
1. После окончательного staging успешно обновите remote refs и зафиксируйте:
   ...
   - OID текущего HEAD как будущего parent;
   - OID merge-base между base и HEAD.
   Покажите полный кандидат PR через `git diff --cached <merge-base-oid> --`;
   при существующем target также покажите изменение относительно него через
   `git diff --cached <target-oid> --`.
2. Получите явное одобрение этого снимка. Непосредственно перед `commit` снова
   обновите remote refs, получите все пять значений и сравните их с одобренными.
   Ошибка обновления или любое изменение запрещает `commit`, `push`, создание
   и обновление PR до нового показа и одобрения.
3. Tree нового commit должен совпасть с одобренным tree, его прямой parent — с
   одобренным HEAD, а merge-base base и commit — с одобренным merge-base. Перед
   `push` снова обновите remote refs, сравните OID удалённых base и target либо
   отсутствие target с одобренными, затем повторите проверки commit.
4. После `push`, но до работы с PR, снова успешно обновите remote refs: OID base
   ...
   Создайте или обновите PR только для этих веток и прочитайте его обратно: OID
   head PR должен совпасть с проверенным OID target, OID base PR — с одобренным
   OID base. Иначе снова покажите кандидата и получите новое одобрение.
