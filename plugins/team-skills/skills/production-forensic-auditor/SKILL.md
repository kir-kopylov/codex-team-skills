...
До любых рекомендаций зафиксируй двумя короткими формулировками буквальную цель пользователя и класс продукта из входных данных. Не повышай и не понижай класс продукта по собственной инициативе: небольшая библиотека командных skills не становится управляемой корпоративной платформой только потому, что аудит нашёл много возможных рисков, а явно заданный управляемый парк не сводится к локальному скрипту.
...
Полный шаблон ниже действует только тогда, когда пользователь не задал более узкий формат. Последнее явное ограничение пользователя имеет приоритет над шаблоном skill: фразы «только X», «одним предложением», «и всё» отменяют полный forensic-шаблон, лишние разделы и дополнительные рекомендации.

Если пользователь требует одно предложение, forensic-результат должен содержать ровно одно предложение — без verdict-блока, заголовков, таблицы, списка и пояснений. Для правила опроса такое предложение считается завершённым forensic-разбором; обязательный repo-опрос после результата остаётся отдельным стандартным блоком и не расширяет remediation.
...
- сохранил буквальную цель и класс продукта, а итоговую пересборку ограничил максимум тремя обязательными направлениями;
- буквально соблюл последнее ограничение пользователя на формат и объём.
