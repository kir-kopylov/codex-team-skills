...
Если наблюдение не входит ни в один заранее пересчитанный прогноз, опыт
валиден, но обе модели отвергнуты: верните `models_rejected`, не формулируйте
поведенческий контракт и следующим шагом пересоберите модели по новому факту.
...
- `models_rejected` — валидное наблюдение не предсказывала ни одна модель;
