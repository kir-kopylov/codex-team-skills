...
description: "Используйте этот экспериментальный skill при первом неоднозначном техническом сбое, когда один симптом допускает несколько причин: «проверь причины сбоя», «какая из двух причин верна», «не гадай, а проверь», «в одном месте работает, в другом нет». Skill отделяет пользовательский результат от косвенных признаков, формулирует две возможные причины и выбирает одну безопасную проверку, для которой они дают разные прогнозы. Самостоятельно выполняет только безопасные проверки; изменения среды требуют отдельного разрешения."
---
...
При автосрабатывании на смысловое сходство сначала извлеките из текущего запроса действие пользователя, конкретную систему и наблюдаемый сбой, запрошенное количество причин или проверок, если оно есть, и проверяемые сведения: точную ошибку, свежие логи, успешный и неуспешный путь, ожидаемый результат и ограничения безопасной диагностики.
...
Для вашей задачи — <кратко повторите действие пользователя, конкретную систему и сбой, запрошенное количество причин или проверок и проверяемые сведения> — может пригодиться командный навык **«Проверка причин сбоя»**.
...
> **С навыком**
>
> <Полными предложениями снова назовите действие пользователя, систему и сбой, количество причин или проверок и проверяемые сведения. Объясните, что наблюдаемый пользовательский результат будет отделён от промежуточных зелёных сигналов, две конкурирующие причины будут сформулированы явно, а затем будет выбран один безопасный опыт, который меняет один причинный фактор и различает эти причины по заранее названным исходам.>

> **Без навыка**
>
> <Полными предложениями снова назовите то же действие пользователя, ту же систему и сбой, количество причин или проверок и проверяемые сведения. Объясните, что обычный ответ может перечислить вероятные причины и команды диагностики, но не гарантирует один причинный контраст, неизменность остальных факторов и заранее определённую интерпретацию результата проверки.>

**Навык не проводит проверку, если она требует неразрешённого изменения рабочей системы, сети, учётных данных, аккаунта, звонка или платежа, а без свежего наблюдения причина останется неизвестной.**
...
desktop-приложении, устройстве или сервисе. Его результат — не правдоподобная
причина и не длинное дерево советов, а один безопасный опыт, наблюдение которого
...
```yaml
outcome: "Какой конечный результат должен увидеть пользователь."
current_observation:
...
target: "Какая техническая система или операция не достигла outcome."
...
2. Зафиксируйте outcome, прямое доказательство успеха и forbidden false
   positives. Открытое окно, зелёный индикатор, живой процесс, TCP-соединение,
   HTTP 200 или успешный локальный тест не заменяют иной пользовательский
   результат.
...
6. Сформулируйте `hypothesis_a` и `hypothesis_b`, которые обе объясняют текущий
   симптом. Назовите наблюдение, по которому их прогнозы расходятся.
...
9. После проверки классифицируйте только полученное наблюдение. Не объявляйте
   root cause по результату `favors_a` или `favors_b`.
10. Покажите пользователю один следующий опыт. Следующую ветку выбирайте только
...
конкурирующими объяснениями одного симптома.
...
- outcome и свежий наблюдаемый симптом названы;
- факты отделены от гипотез и снабжены provenance;
- есть две конкурирующие причины с разными прогнозами;
