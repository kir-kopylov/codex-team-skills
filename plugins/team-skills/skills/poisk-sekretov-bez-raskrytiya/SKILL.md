...
description: "«Найди файлы с паролями», «почту и документы на секреты», «ключи доступа и токены», «карты с PIN, CVV», «только пути к утечкам», «очистка секретов». Path-only, revoke-first."
---
