---
name: peresmotr-predposylok-posle-povtora
description: "«Ходим кругами», «всё мимо», «пересмотри предпосылки», «какой слой», «что сработало», «connected, результата нет», «два раза тот же no-outcome», «больше gates». Смена слоя."
---
...
Смысловое совпадение однозначно только при зацикленной диагностике: названы недостигнутый результат и либо две повторённые неудачные или частичные попытки, либо повторный no-outcome при ложных зелёных сигналах. Одиночный ложный зелёный сигнал не открывает reframe. Одна просьба найти чужой опыт без repair-loop не является смысловым автотриггером: передайте запрос в `kak-drugie-reshili` без запуска этого навыка.

Применяю экспериментальный навык **«Выход из зацикленной диагностики»** (обратная связь — @kir-kopylov): <кратко назовите конкретную пользу для текущего запроса>; продолжаю без ожидания.
...
- когда задача требует только сформулировать новый team skill contract -- это `kontrakt-navyka-do-sborki`;
- когда нужно создать файлы skill в repo -- это `dobavlenie-navyka-v-biblioteku`.
...
```text
Опрос по skill:
1. Что в этом использовании peresmotr-predposylok-posle-povtora было полезно?
2. Что стоит доработать в skill или его формате?
Можно ответить коротко или написать "пропустить".
```

Если пользователь ответил, сохраните санированную карточку в `~/.codex/skill-runs/peresmotr-predposylok-posle-povtora/usage-feedback.jsonl` -- лучше через bundled script:
...
Если пользователь поправил skill, tool/API/browser упал, нарушен режим работы, пришлось искать workaround или skill сделал ложное предположение, запишите приватную карточку в `~/.codex/skill-runs/peresmotr-predposylok-posle-povtora/exception-log.jsonl`.
